...
RAD_COL = 'radiation_daytime_kwh_m2'


def _fit_radiation_yield(rad, y, min_rad: float = 0.05) -> tuple[float, float]:
    """Dopasuj yield = PV / radiacja na train: mediana oraz OLS (bez wyrazu wolnego)."""
...
# Load full data (development + production holdout)
df_full = load_training_frame(
    'data/energy_model.db', dev_start, PRODUCTION_HOLDOUT_END.isoformat(), 'home',
)
df_full['month'] = pd.to_datetime(df_full['day']).dt.month
df_full['year_month'] = pd.to_datetime(df_full['day']).dt.to_period('M').astype(str)

print(f'\nŁącznie danych: {len(df_full)} dni')
print(f'Zakres: {df_full["day"].min()} → {df_full["day"].max()}')
...
print(f'\n📊 DEVELOPMENT SET (CV): {len(dev_df)} dni')
...
dev_df['season'] = dev_df['month'].apply(get_season)
...
for season in ['Wiosna', 'Lato', 'Jesień', 'Zima']:
    ...
    print(f'    {season}: {count} dni ({count/len(dev_df)*100:.1f}%)')

print(f'\n🎯 PRODUCTION HOLDOUT: {len(prod_df)} dni')
...
y_dev = dev_df[target]
groups_dev = pd.to_datetime(dev_df['day']).dt.to_period('M').astype(str)
...
# Baseline rad×yield: yield tylko z fold-train (OOF), nie ze stałej 0.17 ani z testu
print('\n📊 Baseline (rad×yield) — yield z train każdego folda:')
...
for fold_i, (train_idx, test_idx) in enumerate(cv.split(X_dev, y_dev, groups=groups_dev), start=1):
    ...
    print(
        f'  Fold {fold_i}: yield_med={y_med:.3f}, yield_ols={y_ols:.3f} | '
        f'MAE_med={mae_med:.3f}, MAE_ols={mae_ols:.3f}'
    )

baseline_mae = float(np.mean(baseline_fold_mae_med))  # główna metryka = mediana (jak w train_pv_rf_only)
baseline_mae_ols = float(np.mean(baseline_fold_mae_ols))
print(
    f'\n  CV baseline MAE (median yield): {baseline_mae:.3f} ± '
    f'{np.std(baseline_fold_mae_med):.3f} kWh'
)
print(
    f'  CV baseline MAE (OLS yield):    {baseline_mae_ols:.3f} ± '
    f'{np.std(baseline_fold_mae_ols):.3f} kWh'
)
print(
    f'  Yield (mediana foldów): med={np.mean(fold_yields_med):.3f}, '
    f'ols={np.mean(fold_yields_ols):.3f}'
)

print(f'\nUruchamianie CV (5 foldów, może potrwać ~1-2 min)...')
...
print(f'  Poprawa vs baseline (median yield, OOF): {(1 - test_mae.mean()/baseline_mae)*100:.1f}%')
...
print(f'\nTrenuję finalny model na wszystkich {len(dev_df)} dniach...')
...
print(f'\nPredykcja na {len(prod_df)} dniach przyszłości...')
...
# Baseline na holdoucie: yield wyłącznie z development (train), ocena na production
...
print(
    f'\n📊 Baseline na Production (yield z Development): '
    f'med={yield_dev_med:.3f} → MAE={baseline_prod_mae:.3f}; '
    f'ols={yield_dev_ols:.3f} → MAE={baseline_prod_mae_ols:.3f}'
)
...
for month in sorted(prod_df['year_month'].unique()):
    ...
    print(f'\n  {month}: MAE = {month_mae:.3f} kWh ({len(month_data)} dni)')
...
results = pd.DataFrame({
    'Zbiór': [
        'Baseline CV (rad×yield_med, OOF)',
        'CV Mean (5-fold)',
        'Development (full train)',
        'Baseline Prod (yield z Dev)',
        'Production Holdout ⭐',
    ],
    'Liczba dni': [
...
print(f'\n📏 day_length_hours:')
if 'day_length_hours' in feature_cols:
    print(f'   ✅ Dodana i działa (top 10 feature)')
else:
    print(f'   ❌ Nie dodana')
